...
# writerow (escrevendo linha por linha)
with open("filmes.csv", "w", newline='', encoding="utf-8") as arquivo:
    escritor = csv.writer(arquivo)
    escritor.writerow(["Missão impossível 7", 2025, "Ação"])
    escritor.writerow(["Matrix", 1999, "Ficção científica"])

# writerows (escrevendo várias linhas de uma vez)
dados = [
    ['Ana', 30, 'São Paulo'],
...
with open('exemplo.csv', 'w', newline='', encoding="utf-8") as arquivo:
    escritor = csv.writer(arquivo)
